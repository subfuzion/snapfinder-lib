 ...
 * and location (lat, lng) properties
 */
...
/**
 * Reverse geocode a location and callback with zip5, address (formatted address),
 * and location (lat, lng) properties
 */
...
function getLocalAddress(result) {
  var address = ''
    , comps = result.address_components
    , streetNumber = findAddressComponent(comps, 'street_number')
    , address1 = findAddressComponent(comps, 'route')
    , city = findAddressComponent(comps, 'locality')
    ;
